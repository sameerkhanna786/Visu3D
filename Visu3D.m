...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []); % creates arrays with elements listed in parentheses
if nargin && ischar(varargin{1})
    gui_State.gui_Callback = str2func(varargin{1}); % sets gui_State.gui_Callback to be the function handle (allows one to call that function even outside its working region) of the scalar string varargin{1}
end
...
% NOTE: varargin stands for Variable-length input argument list.  

if size(varargin,2)>0 
handles.iM=varargin{1};
handles.iM=handles.iM(:,:,:,:); % what does this mean?
else
    load mri
    handles.iM=double(permute(D,[2 1 4 3])); % permute rearranges dimensions of D so that they are in the order given by vector [2, 1, 4, 3]
end    

handles.dimension=[1 1 2.5]; % sets dimensions to [1, 1, 2.5]
handles.Show_pos=0;
handles.Wait=0;
handles.save_im=0;
handles.filen='';
% Initially sets variables to 0 or blank.

nam=fieldnames(handles); % nam is set to a cell array of strings containing the names of the fields in handles
if sum(strcmp(nam,'x'))==0 % strcmp is 1 if nam and 'x' are identical and 0 otherwise. sum then sums up all of the elements of the resulting vector
handles.x=floor((size(handles.iM,1)+1)/2);
handles.y=floor((size(handles.iM,2)+1)/2);
handles.z=floor((size(handles.iM,3)+1)/2);
end
% WHEN is handles.x defined?
if handles.x>size(handles.iM,1) % size determines the length of handles.iM in dimension 1
handles.x=floor((size(handles.iM,1)+1)/2);
handles.y=floor((size(handles.iM,2)+1)/2);
handles.z=floor((size(handles.iM,3)+1)/2);    
end
...
function varargout = Visu3D_OutputFcn(hObject, eventdata, handles) % Nothing eventful occurs in this function as nothing gets returned from command line (entire program uses GUI for input).
...
% overwrites handles.echo_num multiple times?
...
txt(1)={[txt{1} ', Value: ' num2str(iM(handles.x,handles.y,handles.z,handles.echo_num))]};
...
if nor==2
ab=abs(iM(:,:,:,handles.echo_num));
end
if nor==3
abx=abs(iM(handles.x,:,:,handles.echo_num));
aby=abs(iM(:,handles.y,:,handles.echo_num));
abz=abs(iM(:,:,handles.z,handles.echo_num));
ab=[abx(:);aby(:);abz(:)];
end
ma=max(ab(~isnan(ab)));
if size(ma,1)==0
...
hold on
if handles.Show_pos
    plot([0.5 size(iM,1)+0.5],size(iM,3)+0.5-[handles.z handles.z],'-r')
    plot([handles.x handles.x],[0.5 size(iM,3)+0.5],'-r')
end

if cont
    [X,Y]=meshgrid(1:size(im,2),1:size(im,1));
    v=floor((min(im(:)):lev:lev+max(im(:)))/lev)*lev;
    if (size(im,1)>2).*(size(im,2)>2)
    contour(X,Y,im,v,'-k');
    end
end
...
%     set(gcf,'position',[pos([1 2]) size(im,2) size(im,1)])
...
hold on
if handles.Show_pos
    plot([0.5 size(iM,2)+0.5],size(iM,3)+0.5-[handles.z handles.z],'-r')
    plot([handles.y handles.y],[0.5 size(iM,3)+0.5],'-r')
end
if cont
    [X,Y]=meshgrid(1:size(im,2),1:size(im,1));
    v=floor((min(im(:)):lev:lev+max(im(:)))/lev)*lev;
...
hold on
if handles.Show_pos
    plot([0.5 size(iM,1)+0.5],size(iM,2)+1-[handles.y handles.y],'-r')
    plot([handles.x handles.x],[0.5 size(iM,2)+0.5],'-r')
end
...
if ok % If ok is what? Is this always true?
% [x,y,a,hasimage] = getimage;
[xi,yi] = getline(gcf,'closed'); 
bw=roipoly(im,xi,yi); % Creates a tool that allows user to create a polygon that is saved as an image and acts as a mask
plot(xi,yi)
